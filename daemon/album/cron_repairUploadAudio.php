<?php
/**
 * 修复mediapath存在，但解析音频时长和大小错误的修复进程
 */
include_once (dirname ( dirname ( __FILE__ ) ) . "/DaemonBase.php");
class cron_repairUploadAudio extends DaemonBase
{
    protected $processnum = 1;
    protected function deal() {
        $this->repairUploadAudio();
        exit;
    }
    ...
    protected function repairUploadAudio() {
        // 更新故事为本地地址
        $story = new Story();
        $story_list = $story->get_list("`mediapath`!='' and `file_size`=0 order by id", 10);
        foreach ($story_list as $k => $v) {
                ...
                if (is_array($r) && $r) {
                    ...
                    $this->writeLog("repair故事 {$v['id']} => mediapath 更新成功");
                } else {
                    $this->writeLog("repair故事 {$v['id']} => mediapath 更新失败");
                }
